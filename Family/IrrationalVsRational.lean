...
theorem pi_irrational : Irrational Real.pi := Real.pi_irrational

-- Direct: if π/10 = q ∈ ℚ then π = 10q ∈ ℚ, contradicting pi_irrational
...
noncomputable def dist_real (x : ℝ) : ℝ := min (frac_real x) (1 - frac_real x)
...
theorem alpha0_dist_pos (n : ℕ) (hn : n ≠ 0) : dist_real (n * alpha0) > 0 := by
  ...
  -- frac is in (0,1) because n·α₀ is not an integer
  have hnn  : 0 ≤ frac_real (n * alpha0) := by
    unfold frac_real; linarith [Int.floor_le ((n : ℝ) * alpha0)]
  have hlt  : frac_real (n * alpha0) < 1 := by
    unfold frac_real; linarith [Int.lt_floor_add_one ((n : ℝ) * alpha0)]
  have hne0 : frac_real (n * alpha0) ≠ 0 := by
    unfold frac_real
    intro h
    have heq : (n : ℝ) * alpha0 = ⌈(n : ℝ) * alpha0⌉ := by linarith
    apply hirr
    exact ⟨Int.floor ((n : ℝ) * alpha0) / n,
...
theorem desert_192_1000_empty :
    ((List.range (1000 - 192)).map (· + 192)).all (fun p => !is_exceptional p) = true := by
  native_decide
